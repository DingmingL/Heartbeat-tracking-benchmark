...
from utils import out_pattern, read_segmented_frames_pyav, read_segmented_video_pyav, set_seed, parse_args, read_video_pyav, load_frame_annotations, \
    get_frame_range_for_video, load_prompt, render_prompt, get_target_indices, read_frames_pyav

    
def load_llava_interleave_model(model_id = "llava-hf/llava-interleave-qwen-7b-hf", ban_tokens=None):
...
def infer_one_video(frames, processor, model, task_type, bad_words_ids=None, \
    ...
    for msg in conversation:
        if msg.get("role") == "user":
            # content here is already a list
            msg["content"].extend([{"type": "image"} for _ in range(NUM)])
...
def main():
    ...
    model, processor, bad_words_ids = load_llava_interleave_model(model_id = model_id)
